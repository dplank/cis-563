...
void Scene::draw(const GLuint* vbos)
{
    for(std::vector<Primitive*>::iterator iter = m_primitives.begin(); iter != m_primitives.end(); ++iter)
    {
        (*iter)->draw(vbos);
...
void Scene::Plane::draw(const GLuint* vbos) const
{
...
void Scene::Sphere::draw(const GLuint* vbos) const
{
    glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
    // position
    glBindBuffer(GL_ARRAY_BUFFER, vbos[0]);
...
{// TODO: implement line-sphere intersection. you can refer to line-plane intersection.
    ...
    if(v2 < threshold)
    {
        if(v1 >= threshold)
        {// continuous collision handling.
			float r = m_radius + threshold;
			glm::vec3 l = glm::normalize(p2 - p1);
			glm::vec3 pc = p1 - m_center;
			float a = glm::dot(l, pc);
			float d = -a - glm::sqrt(a*a - glm::dot(pc, pc) + r*r);
			intersect = l*d + p1;
