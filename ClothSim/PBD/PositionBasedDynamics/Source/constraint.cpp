...
{// TODO: implement the project function for FixedPointConstraint.
    ...
    float value = glm::length(m_fixd_point - m_vertices->predicted_pos(m_p0));
    if(value < EPSILON)
        return true;
...
{// TODO: implement the project function for StretchConstraint.
	...
	glm::vec3 p = (p1 - p2)/length;
    m_vertices->predicted_pos(m_p1) -= w1/(w1+w2)*d*p * m_stiffness;
    m_vertices->predicted_pos(m_p2) += w2/(w1+w2)*d*p * m_stiffness;
...
{// TODO: implement the project function for CollisionConstraint.
    ...
    glm::vec3 dp0;
    //m_vertices->predicted_pos(m_p0) += dp0 * m_stiffness;
	m_vertices->predicted_pos(m_p0) = m_ref_point;
    return false;
}
