...
public:
	...
	void advectPhi(double dt);
	void computeSurface();
	void initalizePhi();
	void advectY(double dt);
	void reinitialization();
	int hasAccNeighbor(int i, int j, int k);
	double computeSinglePhi(int i, int j, int k, bool sign);

	void savePhi(int i);
	void setPhi(int i);
	void phiInterpolate(double r);
	void computePhi(double &a, double &l, double &r, int &flag, bool &leftDir);
...
protected:
	...
	double getPhi(const vec3& pt);
	vec3 getCenter(int i, int j, int k);

	// Sets up the A matrix:
	void setUpAMatrix();
	void setUpAMatrixOutside(GridData mFlag);
	...
	GridData mN_z;
	GridData mY;
	GridData mStatus; // 1.0 for accepted band; -1.0 for tentative band; 0.0 for else

	GridData phiFrame[frameNo];
	// The A matrix:
