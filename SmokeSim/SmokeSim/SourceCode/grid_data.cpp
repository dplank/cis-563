...
double GridData::cubic(const double a, const double fkm, const double fk, const double fkp, const double fkpp){
	double delta = fkp - fk;
	double dk = (fkp - fkm)/2;
	double dkp = (fkpp - fk)/2;
	if(dk*delta<=0) dk = 0;
	if(dkp*delta<=0) dkp = 0;
	double f = (dk+dkp-delta)*a*a*a + (3*delta-2*dk-dkp)*a*a + dk*a + fk;
	return f;
}
...
double GridData::interpolate(const vec3& pt)
{
	...
	//if(i>0 && i<theDim[0]-2 && j>0 && j<theDim[1]-2 && k>0 && k<theDim[2]-2) {
	//	int index = 0;
	//	double a = pos[1] - j*theCellSize;
	//	for(int m=i-1;m<i+3;m++){
	//		for(int n=k-1;n<k+3;n++){
	//			tmp1[index] = cubic(a,(*this)(m,j-1,n),(*this)(m,j,n),(*this)(m,j+1,n),(*this)(m,j+2,n));
	//			index ++;
	//		}
	//	}

	//	index = 0;
	//	a = pos[0] - i*theCellSize;
	//	for(int m=0;m<16;m+=4){	
	//		tmp2[index] = cubic(a,tmp1[m],tmp1[m+1],tmp1[m+2],tmp1[m+3]);
	//		index ++;
	//	}

	//	a = pos[2] - k*theCellSize;
	//	tmp = cubic(a,tmp2[0],tmp2[1],tmp2[2],tmp2[3]);
	//} else {
	...
	//	// Z
	//	tmp = LERP(tmp1234, tmp5678, fractz);
	//}
